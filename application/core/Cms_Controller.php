...
class Cms_Controller extends Base_Controller
{
	...
	public function dropdown_opt_generate($opt_name="",$opt_key="",$opt_caption="",$opt_table="",$opt_add="",$opt_condition="",$opt_select2=FALSE,$opt_all=FALSE,$opt_order="",$opt_tambah1="",$opt_tambah2="") //tambah
	{
		if (empty($opt_name)) { return FALSE; }
		if (empty($opt_key)) { return FALSE; }
		if (empty($opt_caption)) { return FALSE; }
		if (empty($opt_table)) { return FALSE; }

		if ($opt_select2 === TRUE) { $list[''] = ''; } else { $list[''] = 'Pilih'; }
		if ($opt_all == TRUE) { $list['all'] = 'ALL'; }
		$this->db->select($opt_key);
		$this->db->select($opt_caption);
		if (!empty($opt_add)) { $this->db->select($opt_add); }
		if (!empty($opt_tambah1)) { $this->db->select($opt_tambah1); }
		if (!empty($opt_tambah2)) { $this->db->select($opt_tambah2); }
		$this->db->from($opt_table);
		if (!empty($opt_condition)) { $this->db->where($opt_condition); }
		if (!empty($opt_order)) { $this->db->order_by($opt_order); }
		$query = $this->db->get()->result_array();
		if (!empty($opt_add)) { foreach ($query as $row) { $list[$row[$opt_key]] = addslashes($row[$opt_add]) ." - ". addslashes($row[$opt_caption]); } }
		else if (!empty($opt_tambah1)) { foreach ($query as $row) { $list[$row[$opt_key]] = addslashes($row[$opt_caption]) ." <> ". addslashes($row[$opt_tambah1]) ." - ". addslashes($row[$opt_tambah2]); } }
		else {foreach ($query as $row) { $list[$row[$opt_key]] = addslashes($row[$opt_caption]); }}

		Template::set($opt_name, $list);
	}

		public function dropdown_optgroup_generate($opt_name="",$opt_key="",$opt_caption="",$opt_table="",$opt_group="",$opt_order="",$opt_add="",$opt_condition="" ) //tambah
		{
			if (empty($opt_name)) { return FALSE; }
			if (empty($opt_key)) { return FALSE; }
			if (empty($opt_caption)) { return FALSE; }
			...
			if (!empty($opt_group)) { $this->db->select($opt_group.' as group'); }
			
			if (!empty($opt_add)) { $this->db->select($opt_add); }
			$this->db->from($opt_table);
			if (!empty($opt_condition)) { $this->db->where($opt_condition); }
			if (!empty($opt_group)) { $this->db->order_by($opt_group,'asc'); }
			if (!empty($opt_order)) { $this->db->order_by($opt_order,'asc'); }
