...
class Settings extends Admin_Controller
{
	...
	function index()
	{
		...
		$this->db->select('a.*, b.apps, b.caption module, c.caption menu', false);
		$this->db->from('permissions a');
		$this->db->join('menu_module b', 'b.module_id=a.module_id');
		$this->db->join('menu_type c', 'c.type_id=a.type_id');
		$this->db->where('b.apps', 'HR');
	...
	public function create()
	{
		...
		$this->db->select('module_id,caption');
		$this->db->from('menu_module');
		$this->db->where('apps', 'HR');
		$this->db->order_by('caption');
		$records = $this->db->get()->result();
		$module_list[''] = 'Pilih Menu Module';
        foreach ($records as $row) { $module_list[$row->module_id] = $row->caption; }
		Template::set('module_list', $module_list);

		$this->db->select('type_id,caption');
		$this->db->from('menu_type');
		$this->db->order_by('caption');
		$records_type = $this->db->get()->result();
		$type_list[''] = 'Pilih Menu Type';
        foreach ($records_type as $row) { $type_list[$row->type_id] = $row->caption; }
		Template::set('type_list', $type_list);
	...
	private function save_permissions($type='insert', $id=0)
	{
		...
		$this->form_validation->set_rules('name','lang:bf_name','required|trim');
