...
class User_model extends BF_Model
{
    ...
    public function employee($users_id)
    {
        $this->db->select("*");
        $this->db->from("x_hrd_employee");
        $this->db->where('users_id', $users_id);
        $query = $this->db->get()->row();
        return $query;
    }

    public function cost_control($company_code)
    {
        $this->db->select("a.*");
        $this->db->from("x_hrd_companies a");
        $this->db->join("x_hrd_companies b","b.header = a.id");
        $this->db->where('b.comp_code', $company_code);
        $query = $this->db->get()->row();
        return $query;
    }    

    public function employee_mapping($nik)
    {
        $this->db->select("*");
        $this->db->from("x_hrd_employee_accounting");
        $this->db->where("nik", $nik);
        $query = $this->db->get()->row();
        return $query;
    }   

    public function update_status($users_id,$condition)
    {
        if ($condition == 'on') 
        {
            $status = 1;
        ...
        $hasil = $this->db->update($this->table_name, $update);
        return true;
    }
