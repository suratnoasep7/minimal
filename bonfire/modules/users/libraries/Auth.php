...
class Auth
{
    ...
    public function login($login, $password, $remember = false)
    {
        ...
        // Check Line
        if ($user->line == "0") {
            Template::set_message('Anda Belum Mendaftarkan Akun LINE silahkan tanyakan ke Korcab Masing-masing!!!','error');
            return false;
        }
        ...
        //ambil data employee
        $emp_query  = $this->ci->user_model->employee($user->id);
        //ambil data cost control
        $cc_query   = $this->ci->user_model->cost_control($emp_query->company_code);
        //ambil mapping
        $mp_query   = $this->ci->user_model->employee_mapping($user->nik);
        //update status
        $this->ci->user_model->update_status($user->id,'on');
        ...
        $this->setupSession(
            ...
            '',
            $user->username,
			$user->login_destination,
			$user->cabang,
			$user->nik,
            $emp_query->company_id,
            $emp_query->company_code,
            $cc_query->comp_code,
            $mp_query->cost_control,
            $emp_query->nama
        );
    ...
    public function logout()
    {
        ...
        $this->ci->user_model->update_status($this->user_id(),'off');
