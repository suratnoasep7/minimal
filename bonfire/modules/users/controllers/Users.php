...
class Users extends Cms_Controller
{
    ...
    public function __construct()
    {
        ...
        // $this->load->model('sms/email_outbox_model');
    ...
    public function login()
    {
        ...
        if (
			...
			*/
            ...
            $this->session->set_userdata('identitys',$this->session->userdata('identity'));
        ...
        Template::set('toolbar_title', 'Halaman Login');
        Template::render('login');
    }
    ...
    public function register()
    {
        ...
        Template::set('languages', unserialize($this->settings_lib->item('site.languages')));
        Template::set('toolbar_title', 'Halaman Register');
        Template::render('login');
    }
    ...
    public function forgot_password()
    {
        ...
        if (isset($_POST['send'])) {
            ...
            $this->db->select("a.nama, b.email, b.telepon, c.id");
            $this->db->from("x_hrd_employee a");
            $this->db->join("x_hrd_employee_detail b", "a.nik = b.nik");
            $this->db->join("users c", "a.users_id = c.id");
            $this->db->where("b.telepon", $email);
            $this->db->or_where("b.email", $email);
            $this->db->where("c.deleted", "0");
            $row = $this->db->get()->row();
            if ($row) {
                $newpass    = $row->id . "-" . rand(pow(10, 3), pow(10, 4) - 1);
                $val        = $this->auth->hash_password($newpass);
                $update     = array("password_hash" => $val['hash']);
                $done       = $this->user_model->update($row->id, $update);
                $message    = "Dear " . $row->nama . " Password Baru anda : " . $newpass . ", Berlaku setelah " . tanggal(date("Y-m-d")) . " Pukul " . date("H:i:s") . " PERINGATAN : JANGAN bagikan password anda kepada SIAPA PUN!!!";
                $on = date('Y-m-d H:i:s', time());
                $insert_email = array(
                    "kategori" => "PASSWORD",
                    "ticketing_order_kode" => "0",
                    "nama" => $row->nama,
                    "email" => $row->email,
                    "subjek" => "PASSWORD BARU NAIKBNK.LINK " . $on,
                    "pesan" => $message,
                    "modified_on" => $on
                );

                $params_update_elearning    = array(
                    'id' => $row->id,
                    'password' => $val['hash']
                );
                $hasil      = $this->curl->simple_post('https://learning.naikbnk.link/api/updatepassword', $params_update_elearning, array(CURLOPT_SSL_VERIFYHOST => 0, CURLOPT_SSL_VERIFYPEER => 0));

                $this->email_outbox_model->insert($insert_email);
                //Modules::run('api/whatsapp/send', $email, $row->telepon, $message);
                Template::set_message('Silahkan Cek Email Anda, Tunggu Hingga 5 Menit jika Belum Masuk ke Email, Segera Hubungi IT via Whatsapp', 'success');
                Template::redirect(LOGIN_URL);
            } else {
                $this->db->select("*");
                $this->db->from("users");
                $this->db->where("whatsapp", $email);
                ...
                if ($row) {
                    ...
                    $hasil      = $this->curl->simple_post('https://learning.naikbnk.link/api/updatepassword', $params_update_elearning, array(CURLOPT_SSL_VERIFYHOST => 0, CURLOPT_SSL_VERIFYPEER => 0));
                    $this->email_outbox_model->insert($insert_email);
                    //Modules::run('api/whatsapp/send', $email, $row->whatsapp, $message);
                    Template::set_message('Silahkan Cek Email Anda, Tunggu Hingga 5 Menit jika Belum Masuk ke Email, Segera Hubungi IT via Whatsapp', 'success');
                    Template::redirect(LOGIN_URL);
                } else {
                    Template::set_message('Whatsapp atau Email Anda Tidak Terdaftar, Segera Hubungi IT via Whatsapp', 'error');
                }
            }
        }
        ...
        if (isset($_POST['set_password'])) {
            ...
            if ($this->form_validation->run() !== false) {
                ...
                if ($this->user_model->update($this->input->post('user_id'), $data)) {
                    ...
                    //kirim email sukses reset password
                    $data['email_kode']        = uniqid() . time();
                    $data['pesan']            = "Selamat Anda Berhasil Reset Password";
                    $to_email                = base64_encode(str_replace(":", "@", $email));
                    $subject                = base64_encode('Naikbhinneka.com Reset Password Berhasil');
                    $message                = base64_encode($data['email_kode'] . "<|>" . $this->load->view('crm/email/default_pesan', $data, true));
                    $return                    = modules::run('crm/email/sent_queue', $to_email, $subject, $message);
                    Template::redirect(LOGIN_URL);
                }
